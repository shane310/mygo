...
func (s *userService) SignUp(r *model.UserServiceSignUpReq) error {
	...
	// 账号唯一性数据检查
	if !s.CheckPassport(r.Name) {
		return errors.New(fmt.Sprintf("账号 %s 已经存在", r.Name))
	}
	// 昵称唯一性数据检查
...
func (s *userService) SignIn(ctx context.Context, name, password string) error {
	password = Md5Encode(password)
	user, err := dao.User.FindOne("name=? and password=?", name, password)
	if err != nil {
		return err
	}
	if user == nil {
