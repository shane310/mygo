...
// @tags    用户服务
// @produce json
// @param   name formData string true "用户账号"
...
func (a *userApi) SignIn(r *ghttp.Request) {
	...
	if err := service.User.SignIn(r.Context(), data.Name, data.Password); err != nil {
		response.JsonExit(r, 1, err.Error())
	} else {
		response.JsonExit(r, 0, "ok")
	}
...
func (a *userApi) Test(r *ghttp.Request) {
...
func (a *userApi) CheckPassport(r *ghttp.Request) {
	...
	if data.Name != "" && !service.User.CheckPassport(data.Name) {
